...
namespace Scene {
    ...
    void
    SceneReconstructor::getFromTransformation(const cv::Mat &transformation, cv::Mat &rotation, cv::Mat &translation) {
    ...
    void SceneReconstructor::computPoseAndProjection(const cv::Mat &localRotation, const cv::Mat &localTranslation,
                                                     const cv::Mat &prevTransform, cv::Mat &globalTransformation,
                                                     cv::Mat &projection) {
    ...
    void SceneReconstructor::reconstructScenes(Iterator<Scene::ImagePair> *pairSequence) {
        ...
        while (pairSequence->hasNext()) {
            ...
            cv::Mat essentialMatrix = cv::findEssentialMat(leftMatches, rightMatches,
                                                           this->calibration.getCameraMatrix(), cv::RANSAC, 0.999, 1,
                                                           matchMask);

            // compute R and t from the essential matrix
            std::vector<uchar> poseMask;
            cv::Mat localRotation, localTranslation;
            int numberOfInliers = cv::recoverPose(essentialMatrix, leftMatches, rightMatches,
                                                  calibration.getCameraMatrix(), localRotation, localTranslation,
                                                  matchMask);

            cv::Mat projection;
            cv::Mat globalTransform;
            computPoseAndProjection(localRotation, localTranslation, currentScene->getPreviousTransform(),
            ...
            if (currentScene->isFirstImagePair()) {
                ...
                std::map<size_t, cv::Point3f> matchingWorldPoints = currentScene->getMatchingWorldPoints(
                        usedKeypointIndexesForReconstruction);
                ...
                if (matchingWorldPoints.size() < 2) {
                    std::cerr << std::endl
                              << "Not enought referenze points were found to scale the current iamge pair. Stopping reconstruction early."
                              << std::endl;
                    break;
                }
                ...
                for (auto outerMatchPointPair = matchingWorldPoints.begin();
                     outerMatchPointPair != matchingWorldPoints.end()--; outerMatchPointPair++) {
                    cv::Point3f unscaledA = toPoint(worldPoints.col(outerMatchPointPair->first));
                    cv::Point3f scaledA = outerMatchPointPair->second;

                    for (auto innerMatchPointPair = std::next(outerMatchPointPair);
                         innerMatchPointPair != matchingWorldPoints.end(); innerMatchPointPair++) {
                ...
                computPoseAndProjection(localRotation, localTranslation * scale, currentScene->getPreviousTransform(),
                                        globalTransform, projection);
